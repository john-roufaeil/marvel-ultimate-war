...
//		((Champion)this.turnOrder.peekMin()).setCondition(Condition.ACTIVE);
...
//		if(this.turnOrder.isEmpty()) return;
		...
		Champion c = this.getCurrentChampion();
//		if ((c.getCurrentActionPoints() < a.getRequiredActionPoints()) || (c.getMana() <  a.getManaCost()) || a.getCurrentCooldown() != a.getBaseCooldown())
//        	throw new NotEnoughResourcesException("Not enough resources available to cast ability.");
		...
		if (c.getMana() <  a.getManaCost())
			throw new NotEnoughResourcesException("Not enough resources available to cast ability.");
		if (a.getCurrentCooldown() != a.getBaseCooldown())
        	return;
		...
		a.execute(targets);
		// check if any target died after execution
...
//		for (Champion c : )
		...
		if((d == Direction.UP && x==4)||(d == Direction.DOWN && x==0)||(d == Direction.LEFT && y==0)||(d == Direction.RIGHT && y==4))
			throw new InvalidTargetException("Invalid target use.");
...
//		Damageable target = null;
		...
		for(Damageable target : castedOn) {
			if(target.getCurrentHP() == 0) {
				if(target instanceof Cover) {
					board[target.getLocation().x][target.getLocation().y] = null;
				}
